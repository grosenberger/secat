...
class pyprophet:
    def __init__(self, outfile, minimum_monomer_delta, minimum_mass_ratio, maximum_sec_shift, xeval_fraction, xeval_num_iter, ss_initial_fdr, ss_iteration_fdr, ss_num_iter, parametric, pfdr, pi0_lambda, pi0_method, pi0_smooth_df, pi0_smooth_log_pi0, lfdr_truncate, lfdr_monotone, lfdr_transformation, lfdr_adj, lfdr_eps, threads, test):
        ...
        # Apply model in run-specific context
        self.df = data.groupby('confidence_bin').apply(self.apply)

        click.echo("Info: Unique interactions detected before integration:")
        click.echo("%s (at q-value < 0.01)" % (self.df[(self.df['decoy'] == 0) & (self.df['qvalue'] < 0.01)][['bait_id','prey_id']].drop_duplicates().shape[0]))
        click.echo("%s (at q-value < 0.05)" % (self.df[(self.df['decoy'] == 0) & (self.df['qvalue'] < 0.05)][['bait_id','prey_id']].drop_duplicates().shape[0]))
        click.echo("%s (at q-value < 0.1)" % (self.df[(self.df['decoy'] == 0) & (self.df['qvalue'] < 0.1)][['bait_id','prey_id']].drop_duplicates().shape[0]))
        click.echo("%s (at q-value < 0.2)" % (self.df[(self.df['decoy'] == 0) & (self.df['qvalue'] < 0.2)][['bait_id','prey_id']].drop_duplicates().shape[0]))
        click.echo("%s (at q-value < 0.5)" % (self.df[(self.df['decoy'] == 0) & (self.df['qvalue'] < 0.5)][['bait_id','prey_id']].drop_duplicates().shape[0]))

        # Apply model in global context
        self.metadf = data.groupby('confidence_bin').apply(self.apply_combined)

        click.echo("Info: Unique interactions detected after integration:")
        click.echo("%s (at q-value < 0.01)" % (self.metadf[(self.metadf['decoy'] == 0) & (self.metadf['qvalue'] < 0.01)][['bait_id','prey_id']].drop_duplicates().shape[0]))
        click.echo("%s (at q-value < 0.05)" % (self.metadf[(self.metadf['decoy'] == 0) & (self.metadf['qvalue'] < 0.05)][['bait_id','prey_id']].drop_duplicates().shape[0]))
        click.echo("%s (at q-value < 0.1)" % (self.metadf[(self.metadf['decoy'] == 0) & (self.metadf['qvalue'] < 0.1)][['bait_id','prey_id']].drop_duplicates().shape[0]))
        click.echo("%s (at q-value < 0.2)" % (self.metadf[(self.metadf['decoy'] == 0) & (self.metadf['qvalue'] < 0.2)][['bait_id','prey_id']].drop_duplicates().shape[0]))
        click.echo("%s (at q-value < 0.5)" % (self.metadf[(self.metadf['decoy'] == 0) & (self.metadf['qvalue'] < 0.5)][['bait_id','prey_id']].drop_duplicates().shape[0]))
    ...
    def apply_combined(self, detecting_data):
        (result, scorer, weights) = PyProphet(self.classifier, self.xgb_hyperparams, self.xgb_params, self.xeval_fraction, self.xeval_num_iter, self.ss_initial_fdr, self.ss_iteration_fdr, self.ss_num_iter, 'pyprophet_metafeature_id', self.parametric, self.pfdr, self.pi0_lambda, self.pi0_method, self.pi0_smooth_df, self.pi0_smooth_log_pi0, self.lfdr_truncate, self.lfdr_monotone, self.lfdr_transformation, self.lfdr_adj, self.lfdr_eps, False, self.threads, self.test).apply_weights(detecting_data, self.weights)

        df = result.scored_tables[['condition_id','replicate_id','bait_id','prey_id','decoy','d_score','p_value','q_value','pep']]
        df.columns = ['condition_id','replicate_id','bait_id','prey_id','decoy','score','pvalue','qvalue','pep']

        self.plot(result, scorer.pi0, "combined_" + str(detecting_data['confidence_bin'].values[0]))
        self.plot_scores(result.scored_tables, "combined_" + str(detecting_data['confidence_bin'].values[0]))
